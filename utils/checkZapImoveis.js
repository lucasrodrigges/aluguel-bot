const { ZAP_URL } = require("../constants");
const wait = require("./wait");
const fs = require("fs");
const { sendEmail, sendWpp } = require("./sendNotification");

const checkZapImoveis = async (browser) => {
  try {
    const { zap: listings, ...rest } = JSON.parse(fs.readFileSync("houses.json", "utf-8"));

    const page = await browser.newPage();
    await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115.0.0.0 Safari/537.36');
    await page.goto(ZAP_URL, { waitUntil: "domcontentloaded" });
    await wait(10000);
    ...
    console.log('Erro ao executar checkZapImoveis:', error);
  }
};

module.exports = checkZapImoveis;